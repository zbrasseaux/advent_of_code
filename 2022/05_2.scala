...
object advent_of_code_2022_05_2 {
	def main(args: Array[String]): Unit = {
		...
		for (line <- lines) {
			...
			val temp_stack: Stack[Char] = Stack();

			// move crates
			// get from original stack
			for (i <- 0 until number_of_crates) {
				val curr_crate = load(from_stack).pop();
				temp_stack.push(curr_crate);
			}
			// deposit in new stack
			for (i <- 0 until number_of_crates) {
				val curr_crate = temp_stack.pop();
